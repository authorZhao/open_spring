...
//  private static final int opc_monitorexit            = 195;
     ...
     *
     * @param fileName        the class name of the proxy class
     * @param interfaces  proxy interfaces
     * @param accessFlags access flags of the proxy class
     */
    public static byte[] generateProxyClass(final String fileName,
                                     Class<?>[] interfaces,
                                     int accessFlags)
    {
        ProxyGenerator gen = new ProxyGenerator(fileName, interfaces, accessFlags);
     ...
     * @param buff
     */
    public static void writeClassToFile(final String fileName, final byte[] buff){
        File file = new File(fileName);
        try {
            FileUtils.copyInputStreamToFile(new ByteArrayInputStream(buff),file);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    /**
     * jdk.proxy.ProxyGenerator.saveGeneratedFiles就是环境里面的参数，如果设置为true为自动生成代理类并保存到文件中
     * jdk11的源码，
     */
    /*public static byte[] generateProxyClass(final String name,
                                            Class<?>[] interfaces,
